...
//    func cropFace(from image: CGImage, boundingBox: CGRect) -> CGImage? {
...
//    }
    func cropFace(from image: CGImage, boundingBox: CGRect) -> CGImage? {
        ...
        // Adjust the bounding box based on the actual image dimensions
        ...
        // Calculate a square bounding box (optional but recommended for emotion models)
        let sideLength = min(adjustedBoundingBox.width, adjustedBoundingBox.height)
        let centeredX = adjustedBoundingBox.midX - (sideLength / 2)
        let centeredY = adjustedBoundingBox.midY - (sideLength / 2)

        let squareCropRect = CGRect(
            x: max(centeredX, 0),
            y: max(centeredY, 0),
            width: min(sideLength, imageWidth - centeredX),
            height: min(sideLength, imageHeight - centeredY)
        )

        return image.cropping(to: squareCropRect)
    }
}
